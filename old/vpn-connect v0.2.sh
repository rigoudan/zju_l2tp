...
function ppp_alive
{
    # this will cause inconsistent behavior, because when ppp is down, xl2tpd possibly not down.
    # if ps -ef | grep $L2TPD_LAC | grep -v grep > /dev/null; then
    # since here i don't know how to reconnect if ppp is down while xl2tpd not,
    # so here i have to down xl2tpd first and restart xl2tpd to re-establish ppp connection.
    if ! ps -ef | grep xl2tpd | grep -v grep > /dev/null; then
